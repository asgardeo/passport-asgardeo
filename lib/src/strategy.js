...
function Strategy(options, verify) {
  ...
  if (!verify) {
    throw new TypeError('Asgardeo requires a verify function');
  }
  if (!options.issuer) {
    throw new TypeError('Asgardeo requires an issuer option');
  }
  if (!options.authorizationURL) {
    throw new TypeError('Asgardeo requires an authorizationURL option');
  }
  if (!options.tokenURL) {
    throw new TypeError('Asgardeo requires a tokenURL option');
  }
  if (!options.clientID) {
    throw new TypeError('Asgardeo requires a clientID option');
  }
  ...
  if (req.query && req.query.code) {
    function restored(err, ctx, state) {
      ...
      self._oauth2.getOAuthAccessToken(code, params, function (err, accessToken, refreshToken, params) {
        ...
        if (!claims.exp) {
          return self.error(new Error('ID token missing expiration time claim'));
        }
        if (!claims.iat) {
        ...
        if (claims.iss !== self._issuer) {
          return self.fail({message: 'ID token not issued by expected OpenID provider.'}, 403);
        }
        ...
        if (typeof claims.aud === 'string') {
          if (claims.aud !== self._oauth2._clientId) {
            return self.fail({message: 'ID token not intended for this relying party.'}, 403);
          }
        } else {
          ...
          if (claims.aud.length > 1 && !claims.azp) {
            return self.fail({message: 'ID token missing authorizied party claim.'}, 403);
          }
        }
        ...
        if (claims.azp && claims.azp !== self._oauth2._clientId) {
          return self.fail({message: 'ID token not issued to this relying party.'}, 403);
        }
        ...
        if (ctx.nonce && claims.nonce !== ctx.nonce) {
          return self.fail({message: 'ID token contains invalid nonce.'}, 403);
        }
    ...
    function stored(err, state) {
      ...
      if (!state) {
        return self.error(new Error('Asgardeo state store did not yield state for authentication request'));
      }
